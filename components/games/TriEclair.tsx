...
export default function TriEclair() {
  ...
  const CATEGORIES: Category[] = [
    { name: t("tri-eclair.catAnimals"), emoji: "🐾", color: "#5CC7A0", bg: "#B8EDD9", items: ["🐶","🐱","🐸","🦊","🐼","🐨","🦁","🐯","🐻","🦋","🐬","🦄"] },
    { name: t("tri-eclair.catFood"),    emoji: "🍽️", color: "#FF922B", bg: "#FFE0B2", items: ["🍎","🍕","🍦","🍇","🥕","🍩","🌮","🍜","🥗","🍰","🥑","🍓"] },
    { name: t("tri-eclair.catNature"),  emoji: "🌿", color: "#3A9FD4", bg: "#BFE3F5", items: ["🌸","⭐","🌙","☀️","❄️","🌊","🔥","🌈","🍀","🌺","🌻","🌴"] },
    { name: t("tri-eclair.catObjects"), emoji: "🎒", color: "#8E72DB", bg: "#E8E0F8", items: ["🎈","⚽","📚","🎸","🏆","🎨","🚀","🎮","💡","🔑","🎁","🌟"] },
  ];
      ...
      <AnimatePresence mode="wait">
        {phase === "idle" && (
          <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-4">
            ...
            <div className="font-display text-lg font-bold text-[#1E2A38] mb-1">Flash Sort</div>
            <div className="text-sm text-[#7A8BA0] mb-4 max-w-xs mx-auto">
              {t("tri-eclair.idle", { time: ROUND_TIME })}
            </div>
        ...
        {(phase === "playing" || phase === "feedback") && (
          <motion.div key="playing" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <div className="flex justify-between items-center mb-4 text-xs">
              <div className="font-bold text-[#FF922B]">{t("tri-eclair.streak", { n: streak })}</div>
              <div className="font-bold text-[#7DC4E8]">⏱️ {timeLeft}{tg("seconds")}</div>
              <div className="font-bold text-[#5CC7A0]">🏆 {score} {tg("pts")}</div>
            </div>
            ...
            <AnimatePresence>
              {streak >= 3 && phase === "playing" && (
                ...
                >
                  🔥 {t("tri-eclair.streak", { n: streak })} · +{streak * 2} {tg("pts")}
                </motion.div>
              )}
            </AnimatePresence>
          </motion.div>
        ...
        {phase === "gameover" && (
          ...
          >
            ...
            <div className="font-display text-xl font-extrabold text-[#1E2A38] mb-1">{tg("timeUp")}</div>

            <div className="grid grid-cols-3 gap-2 my-4">
              {[
                { label: t("tri-eclair.scoreLabel"),     value: score + " " + tg("pts"), color: "#FF922B" },
                { label: t("tri-eclair.accuracyLabel"),  value: accuracy + "%",          color: "#5CC7A0" },
                { label: t("tri-eclair.bestStreakLabel"), value: "x" + bestStreak,        color: "#8E72DB" },
              ].map((s) => (
